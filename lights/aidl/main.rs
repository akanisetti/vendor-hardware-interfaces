...
const LOG_TAG: &str = "lights_service_intel_rust";

use log::LevelFilter;

fn is_debuggable() -> bool {
    rustutils::system_properties::read("ro.debuggable")
	.ok()
	.flatten()
	.map(|v| v == "1")
	.unwrap_or(false)
...
fn main() {
    let log_level = if is_debuggable() {
	LevelFilter::Debug
    } else {
	LevelFilter::Info
    };
    let logger_success = logger::init(
        logger::Config::default().with_tag_on_device(LOG_TAG).with_max_level(log_level),
    );
    if !logger_success {
